...
ui <- page_sidebar(
  ...
  #ShinyJS used to inactivate "Region" input option for the "Regional Comparisons" tab
  useShinyjs(),
  ...
  navset_tab(
    ...
    nav_panel("Sales Trends",
      card(card_header(NULL)),
      card(card_header("Sales Over Time"), 
    ...
    nav_panel("Regional Comparisons",
      card(card_header(NULL)),
      card(
        card_header("Monthly Sales By Region"),
        plotOutput("region_month_hists", height=600)
      )
    )
  )
)